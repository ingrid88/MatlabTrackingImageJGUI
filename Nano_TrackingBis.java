	...
	extends PlugInFrame
{
	...
	public void windowActivated(WindowEvent e) 
	{
		...
		/*if the first cross coorelationship*/
    }
 	...
 	private void updateWindow()
	{
 	...
 	/* Pick a pillar from the image and use in cross-correlation*/
 	private void pickPoints(){

// get ROI coordinates
roi = mainImage.getRoi();
//IJ.log(""+roi);
roi2 = mainImage.getRoi();
Polygon p = roi.getPolygon(); 
	for (int i=0; i<p.npoints; i++) {
		//IJ.log("X:"+p.xpoints[i]+",Y:"+p.ypoints[i]); 
		values_x[jj]=p.xpoints[i];
		values_y[jj]=p.ypoints[i];
		int s = jj+1;
		IJ.log(s+"  "+values_x[jj]+"   "+values_y[jj]);
		//IJ.log("values:"+values_y[jj]); 
		jj++;
		//draw in ip2 the coordinates
	 }


 		//draw();
 	/*2. save the point that was chosen using the *point* or multi-point tool*/

	
 	/*3. draw point and point label on the image*/
 		
	}
 	
 	/* Remove a pillar x,y position for analysis */
 	private void removePoints(){
	/*remove point from values and so on */
		jj = jj-1;
	/*Close Log*/
		IJ.selectWindow("Log");
	...
	for (int i=0; i<jj; i++) {
		int s = i+1;
		IJ.log(s+"  "+values_x[i]+"   "+values_y[i]);
	}
	

	}
 	
 	/* Save the image of points and labels */
 	private void savePointsImage(){
	
ImagePlus image = WindowManager.getCurrentImage();
ImageProcessor ip = image.getProcessor();
	// duplicate the slice
ImageProcessor ip2 = ip.duplicate();
test = new ImagePlus("points", ip2);
test.show();
	//draw on ip2
	IJ.log(""+jj);
 	for (int i=0; i<jj; i++) {
		int s = i+1;
		int x_roi = values_x[i];
		int y_roi = values_y[i];
		//drawPoint(Graphics g, x_roi, y_roi, int n) {
		//roi2 = new Roi(x_roi,y_roi);
		IJ.log("test"+x_roi);
		//roi2.setLocation(x_roi,y_roi);
		IJ.log("test");
		//IJ.log("roi: "+roi2);
		//ip2 draw();
	} 
	int x_roi = values_x[1];
	int y_roi = values_y[1];
	
	//roi2 = pointroi.PointRoi(x_roi,y_roi);
		//IJ.log("test"+x_roi);
	//roi2.setLocation(x_roi,y_roi);
	IJ.log("test1"+x_roi);
	IJ.log("test2"+roi2);
	// 2. save the log	
		String logset = IJ.getLog();
		String imgdir = IJ.getDirectory("image");
		ImagePlus imagename2 = IJ.getImage();
		String title = imagename2.getTitle();
		String tester = imgdir+title;
		String tester2 = tester.substring(0,tester.lastIndexOf("."));
		String filename = tester2+"_points.txt";
		//IJ.showMessage("My_Plugin"," title: "+tester2);
		String sup = IJ.saveString(logset,filename);
		/*IJ.showMessage("",""+incrementer);*/
		/*Close Log*/
		IJ.selectWindow("Log");
		//IJ.runMacro("run('Close')");
	...
	/*Set ROI from the centroid array (choosing roughly 50px each way increase in diameter size from kernel image*/
	private void setNewRoi()
	{
	//replace the roi that is taken from the image here with the one given by the points
	//IJ.log("incrementer:"+incrementer);
	int x = values_x[incrementer-1];
	//IJ.log("x_val"+x);
	int y = values_y[incrementer-1];
	int x_roi = x-30;
	int y_roi = y-30;
	
	int width_roi = 60;
	int height_roi = 60;
	roi = new Roi(x_roi,y_roi,width_roi,height_roi);
	mainImage.setRoi(roi);
	//IJ.log("Roi: "+ roi);
		//roi = mainImage.getRoi();
		if (roi == null) 
		...
		currRoiLabel5.setText(getPillarString());		
	}

	/* recursive through the given container, disabling/enabling all of the
	   children components */
	...
	private String getPillarString()
	{
	return new String(""+incrementer);
	
	}
	/* controller method for the cross correlation.
	   steps through the appropriate slices and calls processCurrentSlice() to
	...
	setNewRoi();
		...
		/* make sure pillars have been chosen*/
		if(values_y == null){
		JOptionPane.showMessageDialog(thisFrame, 
					"Please pick pillars first.", "Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	private void crossCorrDone()
	{
		...
		String tester = imgdir+title;
		String tester2 = tester.substring(0,tester.lastIndexOf("."));
		String filename = tester2+"_"+incrementer+".txt";
		//IJ.showMessage("My_Plugin"," title: "+tester2);
		String sup = IJ.saveString(logset,filename);
		incrementer++;
		/*IJ.showMessage("",""+incrementer);*/
	...
	/* here the actual cross-correlation is performed on the current slice
	   of the image stack. */
	   /*Run processCurrentSlice() as many times as there are kernel images stored in folder from matlab centroid finding*/
	...
	/* 	for (int i=0; i<j; i++) {
		...
		/* step through the ROI using indices i_x and i_y (image_x, image_y) */
		...
		for(int imgY=rect.y; imgY < yLoop; imgY++)
		{
			...
			for(int imgX=rect.x; imgX < xLoop; imgX++)
			{
             ...
             /* compute some numbers eventually useful for setting the threshold*/ 
             float Moyenne= 0;
             float Quadra = 0;
	...
	//realignRoi(maxI + rect.x+ kernWidth/2, maxJ + rect.y+kernHeight/2);
		...
		/* unlock the images within ImageJ String logset = IJ.getLog(); */
		mainImage.unlock();
		kernelImage.unlock();
	}
/* 
	private void buildRoi(int x, int y){
	 //setLocation
	 //setRoi
	} */
 ...
 */
	...
	private void createWindow()
	{
		...
		/******************** Pillar Tracking panel *************************/
		/*Pick Points*/
		JButton pickPointsButtons = new JButton("Choose");
		/*declare how many values inside of arrays*/
			values_x = new int[70];	
			values_y = new int[70];	
		pickPointsButtons.addActionListener(new ActionListener()
		{
		...
		/*Save Image*/
		JButton savePointsButton = new JButton("Save");
		...
		pillarPanel.setLayout(new GridBagLayout());
		gbc = new GridBagConstraints();
		
	 	gbc.fill = GridBagConstraints.HORIZONTAL;
		gbc.insets = new Insets(1,2, 1, 1);
		gbc.gridx = 1; gbc.gridy = 1; gbc.gridwidth = 1;
		pillarPanel.add(pickPointsButtons, gbc);
		gbc.gridx = 2;
		
		pillarPanel.add(removePointsButton, gbc);
		gbc.gridx = 3;
		pillarPanel.add(savePointsButton, gbc);
		...
		pillarPanel.setBorder(BorderFactory.createTitledBorder("Pillars"));
		/******************** Pillar Tracking end *********************/
		...
		currRoiLabel5 = new JLabel(getPillarString());
		JLabel currRoiLabel3 = new JLabel("Current ROI:");
		JLabel currRoiLabel4 = new JLabel("Current Pillar:");
		Font f = currRoiLabel1.getFont();
		currRoiLabel1.setFont(f.deriveFont((float)(f.getSize() - 2)));
		currRoiLabel1.setForeground(Color.black);
		currRoiLabel5.setFont(f.deriveFont((float)(f.getSize() - 2)));
		currRoiLabel5.setForeground(Color.black);
		currRoiLabel3.setForeground(Color.black);
		currRoiLabel4.setForeground(Color.black);
		...
		roiPanel.add(currRoiLabel4, gbc);
		gbc.gridx = 1; gbc.gridy = 2; gbc.gridwidth = 1;
		gbc.insets = new Insets(0, 2, 2, 2);
		
		roiPanel.add(currRoiLabel5, gbc);
...
/* 		sliceSlider = new JSlider(JSlider.HORIZONTAL, 1, 
		...
		topControlPanel.setLayout(new GridLayout(1,3)); //1,3 actually
		topControlPanel.add(kernelPanel);
		topControlPanel.add(roiPanel);
		panel.add(pillarPanel);
